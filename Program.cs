...
class Program
{
    static void Main(string[] args)
    {
        ...
        double salaireNetMensuelDecembre = salaireNetMensuel + (salaireNetMensuel * prime);
        ...
        foreach (string mois in listMois)
        {
            switch (mois)
            {
                case "Août":
                    ...
                    break;
                case "Décembre":
                    Console.WriteLine("Le mois de l'année est le {0:C}, Le salaire brut annuel est de {1:C}", mois, salaireNetMensuelDecembre);
                    break;
                default:
